...
import android.geosvr.dtn.servlib.conv_layers.StreamConvergenceLayer.contact_header_flags_t;
import android.geosvr.dtn.servlib.conv_layers.StreamConvergenceLayer.data_segment_flags_t;
...
*/
	...
	private boolean start_next_bundle() {
		...
		Log.d("TEST",link.dest_ip().toString()+"\tlink.queue size:"+link.queue().size()+"\tinflight size:"+link.inflight().size());
	...
	private void check_completed(InFlightBundle inflight) {
		...
		if (!params_.segment_ack_enabled()) {
			...
			//测试bundle在多少个链接里面
			/*BundleDaemon.getInstance().contactmgr().get_lock().lock();
			try {
				Iterator<Link> itr = BundleDaemon.getInstance().contactmgr().links().iterator();
				int num=0;
				while (itr.hasNext()) {
					Link link = itr.next();
					if (link.queue().contains(inflight.bundle())
							|| link.inflight().contains(inflight.bundle())) 
					{
//						Log.w("TEST","BundleDaemon delete_bundle");
						++num;
						Log.e("TEST","bundle is in queue or inflight:"+link.dest_ip());
					}

					
				}
				Log.e("TEST","inflight trasmitt already,num:"+num);

			} finally {
				BundleDaemon.getInstance().contactmgr().get_lock().unlock();
			}*/
			/*Log.e("TEST",contact_.link().dest_ip()+"map list num:"+inflight.bundle().num_mappings());
			
			//这里发送相应的传输完成的信号，规定了是哪个link上的传输完成
			Log.e("TEST",contact_.link().dest_ip()+"post BundleTransmittedEvent");*/
