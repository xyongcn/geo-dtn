...
public class Link implements Serializable {
	...
	public boolean equals(Object o) {
		...
		{
			Link other=(Link)o;
			if(this.dest_ip().equals(other.dest_ip()) && this.remote_port()==other.remote_port() && this.nexthop().equals(other.nexthop()))
			{
				return true;
			}
			else
	...
	public boolean add_to_queue(Bundle bundle, int total_len) {
		...
		try {
			...
			String text = String.format(
					...
					bundles_queued_);
//			Log.d(TAG, text);
			Log.i("TEST",this.dest_ip()+"add bundle to queue,size:"+bundles_queued_);
...
//			Log.d(TAG, text);
			Log.i("TEST", this.dest_ip()+"del bundle from queue,size:"+bundles_queued_);
	...
	public boolean add_to_inflight(Bundle bundle, int total_len) {
		...
		try {
			...
			text = String.format(
					"adding %s to inflight list (length %s)", bundle,
					bundles_inflight_);
//			Log.d(TAG, text);
			Log.i("TEST", this.dest_ip()+"add bundle to inflight,size:"+bundles_inflight_);
	...
	public boolean del_from_inflight(Bundle bundle, int total_len) {
		...
		try {
			...
			String text = String.format(
					...
					bundles_inflight_);
//			Log.d(TAG, text);
			Log.i("TEST", this.dest_ip()+"del bundle from inflight,size:"+bundles_inflight_);
