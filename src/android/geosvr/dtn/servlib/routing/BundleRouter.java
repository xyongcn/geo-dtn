...
import android.geosvr.dtn.servlib.bundling.BundleActions;
import android.geosvr.dtn.servlib.bundling.BundleDaemon;
import android.geosvr.dtn.servlib.bundling.BundleEventHandler;
...
import android.geosvr.dtn.servlib.routing.epidemic.EpidemicRegistration;
import android.geosvr.dtn.servlib.routing.epidemic.queuing.EpidemicQueuing;
import android.geosvr.dtn.servlib.routing.prophet.ProphetBundleRouter;
import android.geosvr.dtn.servlib.routing.prophet.ProphetRegistration;
...
public abstract class BundleRouter extends BundleEventHandler {
	...
	public boolean accept_bundle(Bundle bundle,
		...
		if (bs.quota() != 0 && (GlobalStorage.getInstance().get_total_size()
						+ bundle.payload().length() > bs.quota())) {
			Log.i(TAG,String.format("accept_bundle: rejecting bundle %d since "+ "cur size %d + bundle size %d bytes > quota %d bytes",
