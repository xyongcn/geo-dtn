...
//		}
		...
		//决定是否能够对多个邻居节点发送数据
		//通过bundle的日志状态来判断是否发送过了，这样可能会导致一个bundle发送给多个下一跳时，只发送第一个节点
		//如果不判断会导致重发发送，适合作为epidemic的中间节点
//		boolean[] found = {false};
//		if (bundle.fwdlog().get_latest_entry(ForwardingInfo.state_t.QUEUED, found) != null)//原有的判断，但是不判断是否为多条link上发送的相同的数据
		/*if (bundle.fwdlog().get_latest_entry(ForwardingInfo.state_t.QUEUED, link) != null) 
		{
			Log.i("TEST","BundleAction部分判定为false,bundle取消发送");
			return false;
		}*/
		
		//判断重复，不给上一跳发送
		String remote_id=link.remote_eid().toString();
		if(bundle.fwdlog().checkPrehoblist(remote_id))
			return false;
...
//		Log.e("TEST",link.dest_ip()+":cacel bundle,queue size:"+link.queue().size()+"inflight size:"+link.inflight().size());
