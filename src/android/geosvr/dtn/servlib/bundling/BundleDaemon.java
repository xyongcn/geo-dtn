...
import android.geosvr.dtn.servlib.bundling.BundleProtocol.custody_signal_reason_t;
...
import android.geosvr.dtn.servlib.reg.PingRegistration;
...
//			bundle.expiration_timer().schedule_at(now_Calendar.getTime());
	...
	protected boolean delete_bundle(final Bundle bundle,
		...
		contactmgr_.get_lock().lock();
		try {
			//如果bundle的transmitted消息可以取消其他link上的bundle
			if(bundle.canCancelOtherLinkTransmit())
			{
	...
	protected void event_handlers_completed(BundleEvent event) {
		...
		if (event.type() == event_type_t.BUNDLE_TRANSMITTED) {
			//这里的传输完成是针对某一条link上的，可能别的link上还没有发完，因此需要相应的判断
//			Log.e("TEST",((BundleTransmittedEvent) event).link().dest_ip()+"mplist num:"+((BundleTransmittedEvent) event).bundle().num_mappings());
		...
		if (bundle != null) {
			Log.w("TEST","event_handlers_completed");
		...
		if (event.type() == event_type_t.BUNDLE_EXPIRED) {
			...
			if (num_mappings != 0) {
				Log.w(TAG,String.format("BundleDaemon:event_handlers_completed, expired bundle %s still has %d mappings ",
									bundle, num_mappings));
			}
		}
	...
	protected void handle_bundle_received(BundleReceivedEvent event) {
		...
		bundle.format(buf);
		Log.i(TAG,String.format("BUNDLE_RECEIVED %s bundle id (%d) prevhop %s (%d bytes recvd)",
					source_str, bundle.bundleid(), event.prevhop().toString(), event.bytes_received()));
	...
	protected boolean try_to_delete(final Bundle bundle) {
		...
		if (!bundle.is_queued_on(pending_bundles_)) {
			...
			Log.e(TAG,String.format("try_to_delete( bundle id %d): bundle not in pending list!",
											bundle.bundleid()));
			return false;
		}
