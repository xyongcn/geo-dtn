...
public class BundleList implements Serializable {
	...
	private Bundle del_bundle(final int pos, boolean free) throws BundleListLockNotHoldByCurrentThread {
		...
		//Log.i("Connection","del bundle:"+b.dest().toString());
		try {
			...
			//从storage中删除bundle时，需要注意其他的bundleList中是否被转发完了
			if (free && b.num_mappings()==0)
			{
				//Log.i("Connect","free bundle:"+b.dest().toString());
