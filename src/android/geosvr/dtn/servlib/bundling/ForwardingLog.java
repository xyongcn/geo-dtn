...
import android.geosvr.dtn.servlib.bundling.ForwardingInfo.state_t;
...
public class ForwardingLog extends ArrayList<ForwardingInfo> implements Serializable{
    ...
    final public ForwardingInfo get_latest_entry(state_t state, Link link){
    	
    	lock_.lock();
    	try{
    		String remote_eid=link.remote_eid().toString();
	    	String local_eid=BundleDaemon.getInstance().local_eid().toString();
    	    for (int i = log_.size()-1; i>=0; --i)
    	    {
    	    	//先防重复
    	    	
    	    	/*String log_eid=log_.get(i).getForwardIp();
    	    	if(remote_eid.equals(log_eid) && !log_eid.equals(local_eid))
    	    	{
    	    		Log.i("TEST","ForwardingLog部分判定为重复");
    	    		return log_.get(i);
    	    	}*/
    	        ...
    	        {
//    	    	    found[0] = true;
    	        	Log.i("TEST","ForwardingLog部分原有状态判定退出");
    	    	    return log_.get(i);
    	        }*/
    		}   	
...
//	    found[0] = false;
    ...
    public void add_entry(final Link link, ForwardingInfo.action_t action,
    	...
    	try{
    	    ...
    	    prehoblist.add(link.remote_eid().toString());
    	}finally{
    		lock_.unlock();
    ...
    /**
     * 为解决epidemic路由，出现新邻居时会对发来数据的上一跳发送问题
     */
    HashSet<String> prehoblist=null;
    
    /**
     * 返回true表示再给上一跳发送bundle
     * @param s
     * @return
     */
    public boolean checkPrehoblist(String s)
    {
    	return prehoblist.contains(s);
    }
}
